...
- [x] 4.1 Electron Notification API連携
- [x] 4.2 通知スケジューラー実装
- [x] 4.3 利用終了通知（24h/3h/1h/30m前）
- [x] 4.4 公式入力期限通知（オプション）
- [x] 4.5 通知再計算トリガー実装
- [x] 4.6 通知テスト（17テスト）
...
- [x] 5.1 JSONエクスポート実装（バージョン番号付き）
- [x] 5.2 JSONインポート実装（Zod検証）
- [x] 5.3 インポート前自動バックアップ
- [x] 5.4 エクスポート/インポートテスト（8テスト）

---

## Phase 6: 最終調整・受け入れテスト

- [x] 6.1 E2Eテスト作成（Playwright）
  - [x] Playwright + Electron セットアップ
  - [x] 受け入れ条件に基づくE2Eシナリオテスト（10テスト）
- [x] 6.2 受け入れ条件チェック（要件定義書第十九章）
  - [x] アプリ起動確認
  - [x] タブ構成確認（5タブ）
  - [x] 概略タブ表示確認
  - [x] 登録タブ表示確認
  - [x] 通知タブ表示確認
- [x] 6.3 カバレッジ確認（90%以上目標）
  - [x] email-parser.ts テスト追加
  - [x] schemas.ts テスト追加
  - [x] カバレッジ: lines 97.64%, branches 86.71%, functions 100%, statements 96.44%
- [x] 6.4 パフォーマンス確認
  - [x] ビルドサイズ確認（main: 428KB, renderer: 657KB）
- [x] 6.5 アクセシビリティ確認
  - [x] キーボード操作対応（タブナビゲーション）
  - [x] ARIA属性設定（role="tab", role="tabpanel"等）
- [x] 6.6 最終ビルド・動作確認
  - [x] lint / type-check / test 全通過
  - [x] E2Eテスト全通過（10テスト）
  - [x] プロダクションビルド成功
...
| 4 | 2026-01-23 | 通知スケジューラー完了（17テスト）、再計算トリガー実装 |
| 5 | 2026-01-23 | エクスポート/インポート完了（8テスト）、バックアップ機能実装 |
| 6 | 2026-01-23 | E2Eテスト完了、単体テスト166件+E2E10件全通過 |
