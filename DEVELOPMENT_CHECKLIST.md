...
- [ ] 2.1 コードエンティティ型定義（src/types/）
- [ ] 2.2 Zodスキーマ定義（ランタイム検証）
- [ ] 2.3 状態判定ロジック実装（未使用/使用中/消費済み/期限切れ）
- [ ] 2.4 連続カバレッジ計算ロジック実装
- [ ] 2.5 次に開始すべきコード候補ロジック実装
- [ ] 2.6 永続化層実装（electron-store）
- [ ] 2.7 単体テスト作成（状態判定・カバレッジ計算）
